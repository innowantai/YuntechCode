function F_PlotModeShapFigure_sub(result,type,dataFolder,SavePath,Savename,plotOpen)
...
for target = 1 : allmode 
    ...
    axis([-1.2 1.2 floor(min([dist_1 dist_2]))-10 ceil(max([dist_1 dist_2]))+3])
    Savename(findstr(Savename,'(1)'):findstr(Savename,'(1)')+2) = '';
    Savename(findstr(Savename,'-')) = '_';
    cd(SavePath)
    print(['-f' num2str(1)], '-djpeg', '-r300', ['5_ModeShapeFigure_' Savename '_Fn_' num2str(result{6,target}) '.jpg']);    
    close all    
end
