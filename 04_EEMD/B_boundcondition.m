function [maxima,minimum,openl,openr,exML]=B_boundcondition(maxima1,minimum1,x,r)
maxima=maxima1;
minimum=minimum1;
Lmax=length(maxima);
Lmin=length(minimum);
...
if length(maxima)>=4
   %外插極大值前、後邊界點
   exML = interp1(maxima(1,2:3),maxima(2,2:3),x(1,1),'linear','extrap'); %extrap:外插
...
if dx1 > dx2 %如果1、2點極大值間距大於2、3點 
    %極大值第2點座標與極小值第1點座標相加的一半無條件進位成整數
    %作為邊界的控制點
...
if exML > lomax %外插點是否大於極大值的邊界點
    %極大值第2點座標與極小值第1點座標的一半相加無條件進位成整數
    posi1 = ceil((maxima(1,2)+minimum(1,1))/2);
    if posi1 ~= maxima(1,2) & posi1 ~= minimum(1,2)
         open2 = 1;
    end
...
if  Lmax + Lmin < extrema_point %上、下極值點總數小於10
